% This is a complete design template, it is divided to a set of cells, each
...
% In this template there is no optimization for the coefficient, so it is
...
% Load a SDM pattern for testing purpose
sdm_data    = load('iPatterns/');

% Design input parameters
Fs          = ;         % Sampling frequency
OSR         = ;         % Over Sampling Frequency
Fsignal     = ;         % Input signal frequency
delta_F     = ;         % Transition bandwidth = (cutoff band-pass band)/cutoff band
...
q           = [[22 20 18 16 14 12 10 8]; [22 20 18 16 14 12 10 8]; [22 20 18 16 14 12 10 8]];
...
Filter_Type = '';           % Flag to constrain the filter structure, wether to be a 
...
POT                  = false;% Flag used for the generation of the decimation factor, 
                            % to constrain it to a Power-Of-Two 'POT' factors or not  
                            % true -> POT
                            % false Non-POT
                            % default value = true 
                            
% Design optional flags                            
