...
sdm_data    = load('iPatterns/OSR32_96MHz_3rdOrder_3bit.txt');

% Design input parameters
Fs          = 96e6;        % Sampling frequency
OSR         = 32;           % Over Sampling Frequency
Fsignal     = 200e3;          % Input signal frequency

delta_F     = 0.15;         % Transition bandwidth = (cutoff band-pass band)/cutoff band
rp_tune     = [0.00001 0.0001 0.0005 0.001 0.005 0.01];        % Passband ripples
rc_tune     = [0.00001 0.0001 0.0005 0.001 0.005 0.01];        % Cutoff suppression
...
IBN_penalty     = -4;       % Acceptable penalty in IBN after deciamtion, 
                            ...
                            % e.g.
                            % -70 - (-66) = -4dB
Sig_penalty     = 0.4;      % Acceptable penalty in the signal peak,
...
q           = [[16 15 14 13 12 5]; [16 15 14 13 12 5]; [16 15 14 13 12 8]];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
rp = 0.0005; %0.001; %0.0001; %0.0001; %0.1; %0.001;
rc = 0.00001; %0.00002; %0.00001; %0.01; %0.0005;
...
N_vector = [1 2];
deviation_values = [[0 1 2 6 8 10 11 12 13 14]; [0 1 2 6 8 10 11 12 13 14]; [4 8 9 10 11 14 16 18 20 28]];
...
mixed_filter_coefficients = [coeff_matrix(2,4:size(coeff_matrix,2)); coeff_matrix(15, 4:size(coeff_matrix,2)); coeff_matrix(33, 4:size(coeff_matrix,2))];
...
% dlmwrite('./Exported_Coefficients/First_Stage_Coeffs_96MHz_4Stages.txt', mixed_filter_coefficients(1,:), ';');
% dlmwrite('./Exported_Coefficients/Second_Stage_Coeffs_96MHz_4Stages.txt', mixed_filter_coefficients(2,:), ';');
% dlmwrite('./Exported_Coefficients/Third_Stage_Coeffs_96MHz_4Stages.txt', mixed_filter_coefficients(3,:), ';');
